...
func _split_stack(slot: InventorySlot):
	var half_stack = int(slot.slot_data.current_stack / 2)
	if half_stack <= 0:
		return
	var new_item = slot.slot_data.duplicate(true)
	new_item.current_stack = half_stack
	slot.slot_data.current_stack -= half_stack
...
func _item_dropped_on_slot(from_slot_id: int, to_slot_id: int):
	...
	if (
		from_item != null
		and to_item != null
		and from_item.item_name == to_item.item_name
		and from_item.max_stack_size > 1
	):
		...
		to_slot._fill_slot(
			from_item, from_item != null and equipped_items.get(from_item.equip_type) == from_item
		)
		from_slot._fill_slot(
			to_item, to_item != null and equipped_items.get(to_item.equip_type) == to_item
		)
	_update_all_slot_highlights()
...
func _update_all_slot_highlights():
	for slot in inventory_slots + hotbar_slots:
		if slot.slot_data != null:
			slot._fill_slot(
				slot.slot_data, equipped_items.get(slot.slot_data.equip_type) == slot.slot_data
			)
...
func _drop_data(_at_position: Vector2, data: Variant) -> void:
	...
	if (
		slot.slot_data.equip_type != ItemData.EquipType.NONE
		and equipped_items.get(slot.slot_data.equip_type) == slot.slot_data
	):
...
func _pickup_item(item: ItemData):
	...
	while remaining_stack > 0:
		...
		for slot in all_slots:
			if (
				slot.slot_filled
				and slot.slot_data.item_name == item.item_name
				and slot.slot_data.max_stack_size > 1
			):
				...
				if add_amount > 0:
					...
					slot._fill_slot(
						slot.slot_data,
						equipped_items.get(slot.slot_data.equip_type) == slot.slot_data
					)
		...
		if not new_slot_found and remaining_stack > 0:
			...
			node.global_position = (
				player_body.global_position + player_body.global_transform.basis.x * 2.0
			)
			break
...
func pickup_interactable_item(interactable_item: InteractableItem):
	...
	print(interactable_item.item_data.current_stack)
...
func _get_world_mouse_position() -> Vector3:
	...
	var end = (
		...
		+ (
			...
			* cam.global_position.distance_to(player_body.global_position)
			* 2.0
		)
	)
