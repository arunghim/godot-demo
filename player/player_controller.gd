...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("toggle_inventory") and inventory_ui:
		...
		return
	if inventory_open and event.is_action_pressed("ui_cancel"): Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	if not inventory_open and event.is_action_pressed("left_click"): Input.mouse_mode = Input.MOUSE_MODE_CAPTURED

func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED: camera_input_direction = event.screen_relative * mouse_sensitivity
...
func _physics_process(delta: float) -> void:
	...
	if can_act:
		if Input.is_action_just_pressed("primary_action"): mannequin_animation_player.play("Punch_Cross"); attack_cooldown = mannequin_animation_player.current_animation_length + 0.1
		if Input.is_action_just_pressed("special_action"): mannequin_animation_player.play("Punch_Jab"); attack_cooldown = mannequin_animation_player.current_animation_length + 0.1
		if Input.is_action_just_pressed("interact"): mannequin_animation_player.play("Spell_Simple_Exit"); attack_cooldown = mannequin_animation_player.current_animation_length + 0.1
		if Input.is_action_just_pressed("dodge"):
			is_dodging = true; is_blocking = false
			dodge_direction = last_movement_direction.normalized()
			if dodge_direction == Vector3.ZERO: dodge_direction = -camera.global_basis.z; dodge_direction.y = 0.0
			mannequin_animation_player.play("Roll"); dodge_timer = mannequin_animation_player.current_animation_length

	if Input.is_action_pressed("secondary_action") and not is_dodging and not inventory_open:
		if not is_blocking: is_blocking = true; mannequin_animation_player.play("Punch_Enter"); mannequin_animation_player.seek(mannequin_animation_player.current_animation_length, true)
	...
	if move_direction.length() > 0.0: move_direction = move_direction.normalized()

	var is_sprinting := Input.is_action_pressed("sprint")
	var is_crouching := Input.is_action_pressed("crouch")
	var target_speed := move_speed
	if is_blocking or inventory_open: target_speed = crouch_speed
	...
	if is_dodging:
		...
		dodge_timer -= delta
		if dodge_timer <= 0.0: is_dodging = false
	elif move_direction.length() > 0.001: horizontal_velocity = horizontal_velocity.move_toward(move_direction * target_speed, acceleration * delta)
	else: horizontal_velocity = horizontal_velocity.move_toward(Vector3.ZERO, acceleration * delta)

	velocity.x = horizontal_velocity.x
	velocity.z = horizontal_velocity.z
	velocity.y += gravity * delta
	if Input.is_action_just_pressed("jump") and is_on_floor() and not inventory_open: velocity.y = jump_impulse

	move_and_slide()

	if move_direction.length() > 0.05: last_movement_direction = move_direction
